...
import com.roch.fupin.entity.LiuGeYiPiTongJiDetail_ResultList;
import com.roch.fupin.entity.LiuGeYiPinTongJiDetail;
...
public class LiuGeYiPiTongJiDetailActivity extends MainBaseActivity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (title.contains("大病救治")) {
			leixing = LX.dbjz;
			tv_title.setText("大病救治");
			initData("大病救治");
			case_detail="大病救治";
		}else if (title.contains("社会保障兜底")) {
			leixing = LX.shbzdd;
			tv_title.setText("社会保障兜底");
			initData("社会保障兜底");
			case_detail="社会保障兜底";
		}else if (title.contains("教育扶贫")) {
			leixing = LX.jyfp;
			tv_title.setText("教育扶贫");
			initData("教育扶贫");
			case_detail="教育扶贫";
		}else if (title.contains("产业发展和转移就业")) {
			leixing = LX.cyfzhzyjy;
			tv_title.setText("产业发展和转移就业");
			initData("产业发展和转移就业");
			case_detail="产业发展和转移就业";
		}else if (title.contains("异地扶贫搬迁")) {
			leixing = LX.ydfpbq;
			tv_title.setText("异地扶贫搬迁");
			initData("异地扶贫搬迁");
			case_detail="异地扶贫搬迁";
		}else if (title.contains("生态补偿")) {
			leixing = LX.stbc;
			tv_title.setText("生态补偿");
			initData("生态补偿");
			case_detail="生态补偿";
		}
...
//		System.out.println("贫困人口统计页面一进来初始化时请求服务器中筛选条件的乡镇(flag=7)网址为：==" + URLs.TOWN + "?&ad_cd=" + ad_cd);
	...
	private void getData() {
		...
		if(flag_zhen_cun==1){
			if(StringUtil.isNotEmpty(adl_cd_zhen)){
				...
				MyApplication.getInstance().getHttpUtilsInstance().send(HttpMethod.POST,
						URLs.LiuGeYiPiTongJi_DETAIL, rp, new MyRequestCallBack((BaseActivity) mActivity, MyConstans.FIRST));
				System.out.println("贫困人口统计请求服务器网址为：===" + URLs.LiuGeYiPiTongJi_DETAIL);
			}
	...
	private void initData(String pp_poorreason) {
		...
		MyApplication.getInstance().getHttpUtilsInstance().send(HttpMethod.POST, URLs.LiuGeYiPiTongJi_DETAIL, rp
				, new MyRequestCallBack(mActivity, MyConstans.FIRST));
		System.out.println("请求六个一批统计详情的网址为：" + URLs.LiuGeYiPiTongJi_DETAIL);
	}
	...
	public void onSuccessResult(String str, int flag) {
		...
		switch (flag){
			case MyConstans.FIRST:
				System.out.println("请求六个一批统计详情数据成功：==" + str);
				LiuGeYiPiTongJiDetail_ResultList caseListResult = LiuGeYiPiTongJiDetail_ResultList.parseToT(str, LiuGeYiPiTongJiDetail_ResultList.class);
				if (caseListResult != null && caseListResult.getSuccess()) {
					List<LiuGeYiPinTongJiDetail> list = caseListResult.getJsondata();
...
//								expandTabView.setValue(mTextArray, mViewArray);
	...
	public void onFaileResult(String str, int flag) {
		...
		System.out.println("请求六个一批统计详情数据失败：==" + str);
	}
	 ...
	 * ZhaoDongShao
	 */
	private void generateData(List<LiuGeYiPinTongJiDetail> lEntities) {
...
//		String[] case_bing_laodong_money_jishu = {"行政区","户数","人数"};//因病
		...
		String[] case_dbj = {"行政区","大病救治户数","大病救治人数"};//大病救治
		String[] case_shbzdd = {"行政区","社会保障兜底户数","社会保障兜底人数"};//社会保障兜底
		String[] case_jyfp = {"行政区","教育扶贫户数","教育扶贫人数"};//教育扶贫
		String[] case_cyfzhzyjy = {"行政区","产业发展和转移就业户数","产业发展和转移就业人数"};//产业发展和转移就业
		String[] ydfpbq = {"行政区","异地扶贫搬迁户数","异地扶贫搬迁人数"};//异地扶贫搬迁
		String[] stbc = {"行政区","生态补偿户数","生态补偿人数"};//生态补偿

		switch (leixing) {
			case dbjz:  //大病救治
			getBing(case_dbj, lEntities);
			break;

			case shbzdd: //社会保障兜底
			getXue(case_shbzdd, lEntities);
			break;

			case jyfp: //教育扶贫
			getCan(case_jyfp, lEntities);
			break;

			case cyfzhzyjy: //产业发展和转移就业
			getNengli(case_cyfzhzyjy, lEntities);
			break;

			case ydfpbq: //异地扶贫搬迁
			getMoney(ydfpbq, lEntities);
			break;

			case stbc: //生态补偿
			getJishu(stbc, lEntities);
			break;
	...
	private void getJishu(String[] case_bing_laodong_money_jishu, List<LiuGeYiPinTongJiDetail> lEntities) {
			...
			if (i == 0) {
				...
				for (int j = 0; j < case_bing_laodong_money_jishu.length; j++) {
					...
					case 1:
						row.setGravity(Gravity.CENTER);
						row.setLayoutParams(getLayoutParams(1,1,1,0));
						row.addView(getLinearLayout(getTextView(String.valueOf(lEntities.get(i - 1).getStbc()))));
						break;

					case 2:
						row.setGravity(Gravity.CENTER);
						row.setLayoutParams(getLayoutParams(1,1,1,0));
						row.addView(getLinearLayout(getTextView(String.valueOf(lEntities.get(i - 1).getStbcrs()))));
						break;
	 ...
	 */
	@SuppressLint("InflateParams")
	private void getMoney(String[] case_bing_laodong_money_jishu, List<LiuGeYiPinTongJiDetail> lEntities) {
		for (int i = 0; i < lEntities.size() + 1; i++) {
			row = new TableRow(mContext);
			if (i == 0) {
				for (int j = 0; j < case_bing_laodong_money_jishu.length; j++) {
				...
				for (int j = 0; j < case_bing_laodong_money_jishu.length; j++) {
					...
					case 1:
						row.setGravity(Gravity.CENTER);
						row.setLayoutParams(getLayoutParams(1,1,1,0));
						row.addView(getLinearLayout(getTextView(String.valueOf(lEntities.get(i - 1 ).getYdfpbq()))));
						break;

					case 2:
						row.setGravity(Gravity.CENTER);
						row.setLayoutParams(getLayoutParams(1,1,1,0));
						row.addView(getLinearLayout(getTextView(String.valueOf(lEntities.get(i - 1).getYdfpbqrs()))));
						break;
	 ...
	 */
	@SuppressLint("InflateParams")
	private void getNengli(String[] case_bing_laodong_money_jishu, List<LiuGeYiPinTongJiDetail> lEntities) {
		for (int i = 0; i < lEntities.size() + 1; i++) {
			row = new TableRow(mContext);
			if (i == 0) {
				for (int j = 0; j < case_bing_laodong_money_jishu.length; j++) {
				...
				for (int j = 0; j < case_bing_laodong_money_jishu.length; j++) {
					...
					case 1:
						row.setGravity(Gravity.CENTER);
						row.setLayoutParams(getLayoutParams(1,1,1,0));
						row.addView(getLinearLayout(getTextView(String.valueOf(lEntities.get(i - 1).getCyfzzyjy()))));
						break;

					case 2:
						row.setGravity(Gravity.CENTER);
						row.setLayoutParams(getLayoutParams(1,1,1,0));
						row.addView(getLinearLayout(getTextView(String.valueOf(lEntities.get(i - 1).getCyfzzyjyrs()))));
						break;
	 ...
	 */
	@SuppressLint("InflateParams")
	private void getXue(String[] case_xue, List<LiuGeYiPinTongJiDetail> lEntities) {
		for (int i = 0; i < lEntities.size() + 1; i++) {
			row = new TableRow(mContext);
			if (i == 0) {
				...
				for (int j = 0; j < case_xue.length; j++) {
					...
					case 1:
						row.setGravity(Gravity.CENTER);
						row.setLayoutParams(getLayoutParams(1,1,1,0));
						row.addView(getLinearLayout(getTextView(String.valueOf(lEntities.get(i - 1).getShbzdd()))));
						break;

					case 2:
						row.setGravity(Gravity.CENTER);
						row.setLayoutParams(getLayoutParams(1,1,1,0));
						row.addView(getLinearLayout(getTextView(String.valueOf(lEntities.get(i - 1).getShbzddrs()))));
						break;
	 ...
	 */
	@SuppressLint("InflateParams")
	private void getBing(String[] case_bing_laodong_money_jishu, List<LiuGeYiPinTongJiDetail> lEntities) {
		for (int i = 0; i < lEntities.size() + 1; i++) {
			row = new TableRow(mContext);
			if (i == 0) {
				for (int j = 0; j < case_bing_laodong_money_jishu.length; j++) {
				...
				for (int j = 0; j < case_bing_laodong_money_jishu.length; j++) {
					...
					case 1:
						row.addView(getLinearLayout(getTextView(String.valueOf(lEntities.get(i - 1).getDbjz()))));
						row.setLayoutParams(getLayoutParams(1,1,1,0));
						row.setGravity(Gravity.CENTER);
						break;

					case 2:
						row.addView(getLinearLayout(getTextView(String.valueOf(lEntities.get(i - 1).getDbjzrs()))));
						row.setLayoutParams(getLayoutParams(1,1,1,0));
						row.setGravity(Gravity.CENTER);
						break;
					default:
	 ...
	 */
	@SuppressLint("InflateParams")
	private void getCan(String[] case_can, List<LiuGeYiPinTongJiDetail> lEntities) {
		for (int i = 0; i < lEntities.size() + 1; i++) {
			row = new TableRow(mContext);
			if (i == 0) {
				...
				for (int j = 0; j < case_can.length; j++) {
					...
					case 1:
						row.setGravity(Gravity.CENTER);
						row.setLayoutParams(getLayoutParams(1,1,1,0));
						row.addView(getLinearLayout(getTextView(String.valueOf(lEntities.get(i - 1).getJyfp()))));
						break;

					case 2:
						row.setGravity(Gravity.CENTER);
						row.setLayoutParams(getLayoutParams(1,1,1,0));
						row.addView(getLinearLayout(getTextView(String.valueOf(lEntities.get(i - 1).getJyfprs()))));
						break;
