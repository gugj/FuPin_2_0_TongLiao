...
public class ZhengCeZiXunActivity extends MainBaseActivity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		registerCommentCountReceiver();
	}

	private int currentid=-1;
	private int commentCount;
	private void registerCommentCountReceiver() {
		IntentFilter intentFilter=new IntentFilter("CommentCountBroadcast");
		registerReceiver(new BroadcastReceiver() {
			@Override
			public void onReceive(Context context, Intent intent) {
				String itemId = intent.getStringExtra("itemId");
				String currentCommentCount = intent.getStringExtra("currentCommentCount");

				commentCount = Integer.parseInt(currentCommentCount);
				currentid = Integer.parseInt(itemId);
				System.out.println("评论位置的id="+itemId+"--评论数量为="+currentCommentCount);
			}
		}, intentFilter);
	}
	...
	protected void onResume() {
		super.onResume();
		if (adapter != null) {
			adapter.setCommentCount(currentid,commentCount);
