...
public class LaoDongNengLiLeiXingDetailActivity extends MainBaseActivity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (title.contains("普通劳动力")) {
			leixing = LX.dbjz;
			tv_title.setText("普通劳动力");
			initData("普通劳动力");
			case_detail="普通劳动力";
		}else if (title.contains("丧失劳动力")) {
			leixing = LX.shbzdd;
			tv_title.setText("丧失劳动力");
			initData("丧失劳动力");
			case_detail="丧失劳动力";
		}else if (title.contains("无劳动力")) {
			leixing = LX.jyfp;
			tv_title.setText("无劳动力");
			initData("无劳动力");
			case_detail="无劳动力";
		}else if (title.contains("技能劳动力")) {
			leixing = LX.cyfzhzyjy;
			tv_title.setText("技能劳动力");
			initData("技能劳动力");
			case_detail="技能劳动力";
		}
...
//		System.out.println("贫困人口统计页面一进来初始化时请求服务器中筛选条件的乡镇(flag=7)网址为：==" + URLs.TOWN + "?&ad_cd=" + ad_cd);
	...
	private void getData() {
		...
		if(flag_zhen_cun==1){
			if(StringUtil.isNotEmpty(adl_cd_zhen)){
				...
				MyApplication.getInstance().getHttpUtilsInstance().send(HttpMethod.POST,
						URLs.LaoDongNengLiLeiXingTongJi_DETAIL, rp, new MyRequestCallBack((BaseActivity) mActivity, MyConstans.FIRST));
	...
	private void initData(String pp_poorreason) {
		...
		MyApplication.getInstance().getHttpUtilsInstance().send(HttpMethod.POST, URLs.LaoDongNengLiLeiXingTongJi_DETAIL, rp
				, new MyRequestCallBack(mActivity, MyConstans.FIRST));
		System.out.println("请求劳动能力类型统计详情的网址为：" + URLs.LaoDongNengLiLeiXingTongJi_DETAIL);
	}
	...
	public void onSuccessResult(String str, int flag) {
		...
		switch (flag){
			case MyConstans.FIRST:
				System.out.println("请求劳动能力类型统计详情数据成功：==" + str);
...
//								expandTabView.setValue(mTextArray, mViewArray);
	...
	public void onFaileResult(String str, int flag) {
		...
		System.out.println("请求劳动能力类型统计数据失败：==" + str);
	}
	...
	private void generateData(List<BasicPoorpeopleModel> lEntities) {
		String[] putong = {"行政区","贫困人口数","普通劳动力"};//普通劳动力
		String[] sangshi = {"行政区","贫困人口数","丧失劳动力"};//丧失劳动力
		String[] wu = {"行政区","贫困人口数","无劳动力"};//无劳动力
		String[] jineng = {"行政区","贫困人口数","技能劳动力"};//技能劳动力
			...
			if (i == 0) {
				...
				for (int j = 0; j < case_xue.length; j++) {
					...
					case 2:
						row.setGravity(Gravity.CENTER);
						row.setLayoutParams(getLayoutParams(1,1,1,0));
						row.addView(getLinearLayout(getTextView(String.valueOf(lEntities.get(i - 1).getComchronicdisnum()))));
						break;
