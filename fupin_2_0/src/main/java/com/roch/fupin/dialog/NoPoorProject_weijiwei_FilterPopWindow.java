 ...
 *
 */
public class NoPoorProject_weijiwei_FilterPopWindow extends PopupWindow {
	...
	public NoPoorProject_weijiwei_FilterPopWindow(final Context context) {  
		super(context);
		LayoutInflater inflater = (LayoutInflater) context  
				.getSystemService(Context.LAYOUT_INFLATER_SERVICE);  
		conentView = inflater.inflate(R.layout.no_poor_project_weijiwei_filter_popwindow, null,false);  
	...
	private void initTimePop(final int i) {
		...
		timeSure.setOnClickListener(new OnClickListener() {// 
			@Override
			public void onClick(View v) {
	 ...
	 *
	 */
	public void setAdapter(List<MapEntity> list) {
		// TODO Auto-generated method stub
		if (list.size() > 0) {
			for (int i = 0; i < list.size(); i++) {
				if (list.get(i).getKey().equals("startbaoxiao")) {

					tv_time_start.setText(list.get(i).getValue());

				}else if (list.get(i).getKey().equals("endbaoxiao")) {
