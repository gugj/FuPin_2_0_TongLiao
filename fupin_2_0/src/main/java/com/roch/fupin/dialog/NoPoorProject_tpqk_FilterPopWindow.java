 ...
 * 2016年6月2日
 */
public class NoPoorProject_tpqk_FilterPopWindow extends PopupWindow {
	...
	public NoPoorProject_tpqk_FilterPopWindow(final Context context) {  
		super(context);
		LayoutInflater inflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
		conentView = inflater.inflate(R.layout.no_poor_project_tpqk_filter_popwindow, null,false);  
		...
		btn_submit.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				showMessageListener.Message(tv_time_end.getText().toString().trim().replace("时间不限", "")
						, et_shouru.getText().toString().trim());
			}
		});
	}
	/** 
	 ...
	 * @param list
	 */
	public void setSelectionAdapter(List<MapEntity> list,String selectShouRu){
		if (list.size() > 0) {
	...
	public interface ShowMessageListener {
		/**
		 * @param time_end 截止时间 2016年6月2日
		 * ZhaoDongShao
		 */
		void Message(String time_end,String shouru);
	}
