...
import com.roch.fupin.entity.FuPinZhuanXiangZiJing;
import com.roch.fupin.entity.MapEntity;
import com.roch.fupin.entity.PinKunCun;
import com.roch.fupin.entity.ResultFuPinZhuanXiangZiJing_ResultList;
...
import com.roch.fupin.view.FPZXZJTableScrollView;
 ...
 * 2016年8月11日
 */
@ContentView(R.layout.activity_fupinzhuanxiangzij_tongji)
public class FuPinZhuanXiangZiJinTongJiActivity extends MainBaseActivity implements ShowMessageListener{
	...
	protected List<FPZXZJTableScrollView> mHScrollViews =new ArrayList<FPZXZJTableScrollView>();

	public FPZXZJTableScrollView mTouchView;

	//创建数组保存表头
	private String[] cols = {"title"
			,"中央(自治区)资金","市级资金","旗县级资金"
			,"发展资金(三西资金)","以工代账资金","少数民族发展资金","国有贫困林场资金","国有贫困农场资金","发展资金(管理费)"};
			...
			public boolean onMenuItemClick(MenuItem menuItem) {
				switch (menuItem.getItemId()) {
					case R.id.select:
						...
						filterPopWindow.setShowMessageListener(FuPinZhuanXiangZiJinTongJiActivity.this);
...
//				}
	...
	private void initData() {
		...
		MyApplication.getInstance().getHttpUtilsInstance().send(HttpMethod.POST,
				URLs.FuPinZhuanXiangZiJing_TongJi, rp, new MyRequestCallBack((BaseActivity) mActivity, MyConstans.FIRST));
		System.out.println("贫困人口统计请求服务器网址为：===" + URLs.FuPinZhuanXiangZiJing_TongJi);
	}
...
//		String time = year + "年" + month + "月份";
		String time = year + "年";
		tv_time.setText(time + "扶贫专项资金统计");

		MyApplication.getInstance().getHttpUtilsInstance().send(
				HttpMethod.POST, URLs.FuPinZhuanXiangZiJing_TongJi,
	...
	public void onSuccessResult(String str, int flag) {
		...
		switch (flag){
			case MyConstans.FIRST:
				List<FuPinZhuanXiangZiJing> list = new ArrayList<FuPinZhuanXiangZiJing>();
				ResultFuPinZhuanXiangZiJing_ResultList listResult = ResultFuPinZhuanXiangZiJing_ResultList.parseToT(str, ResultFuPinZhuanXiangZiJing_ResultList.class);
				if (listResult != null ) {
					if (listResult.getSuccess()) {
						...
						if (StringUtil.isNotEmpty(list)) {
							...
							FPZXZJTableScrollView headerScroll = (FPZXZJTableScrollView) findViewById(R.id.item_scroll_title);
							...
							for(int i = 0; i < list.size(); i++) {
								...
								for (int j = 1; j < 10; j++) {
									switch (j) {
										case 1:
											data.put("中央(自治区)资金", String.valueOf(list.get(i).zhongyang));
											break;

										case 2:
											data.put("市级资金", String.valueOf(list.get(i).shiji));
											break;

										case 3:
											data.put("旗县级资金", String.valueOf(list.get(i).qixianji));
											break;

										case 4:
											data.put("发展资金(三西资金)", String.valueOf(list.get(i).fzzjsxzj));
											break;

										case 5:
											data.put("以工代账资金", String.valueOf(list.get(i).ygdzzj));
											break;

										case 6:
											data.put("少数民族发展资金", String.valueOf(list.get(i).ssmzfzzj));
											break;

										case 7:
											data.put("国有贫困林场资金", String.valueOf(list.get(i).gypklczj));
											break;

										case 8:
											data.put("国有贫困农场资金", String.valueOf(list.get(i).gypknczj));
											break;

										case 9:
											data.put("发展资金(管理费)", String.valueOf(list.get(i).fzzjglf));
											break;
							...
							mAdapter = new ScrollAdapter(this, datas, R.layout.listitem_fupinzhuanxiangzij_tongji //R.layout.item
									, cols
...
//								expandTabView.setValue(mTextArray, mViewArray);
	...
	public void addHViews(final FPZXZJTableScrollView hScrollView) {
		if(!mHScrollViews.isEmpty()) {
			int size = mHScrollViews.size();
			FPZXZJTableScrollView scrollView = mHScrollViews.get(size - 1);
	...
	public void onScrollChanged(int l, int t, int oldl, int oldt){
		for (FPZXZJTableScrollView tableScrollView : mHScrollViews) {
	...
	class ScrollAdapter extends SimpleAdapter{
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if(v == null) {
				...
				addHViews((FPZXZJTableScrollView) v.findViewById(R.id.item_chscroll_scroll));
