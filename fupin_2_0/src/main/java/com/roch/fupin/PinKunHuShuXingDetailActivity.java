...
public class PinKunHuShuXingDetailActivity extends MainBaseActivity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (title.contains("五保贫困户")) {
			leixing = LX.dbjz;
			tv_title.setText("五保贫困户");
			initData("五保贫困户");
			case_detail="五保贫困户";
		}else if (title.contains("一般农户")) {
			leixing = LX.shbzdd;
			tv_title.setText("一般农户");
			initData("一般农户");
			case_detail="一般农户";
		}else if (title.contains("一般贫困户")) {
			leixing = LX.jyfp;
			tv_title.setText("一般贫困户");
			initData("一般贫困户");
			case_detail="一般贫困户";
		}else if (title.contains("低保贫困户")) {
			leixing = LX.cyfzhzyjy;
			tv_title.setText("低保贫困户");
			initData("低保贫困户");
			case_detail="低保贫困户";
		}
...
//		System.out.println("贫困人口统计页面一进来初始化时请求服务器中筛选条件的乡镇(flag=7)网址为：==" + URLs.TOWN + "?&ad_cd=" + ad_cd);
	...
	private void getData() {
		...
		if(flag_zhen_cun==1){
			if(StringUtil.isNotEmpty(adl_cd_zhen)){
				...
				MyApplication.getInstance().getHttpUtilsInstance().send(HttpMethod.POST,
						URLs.PinKunHuShuXingTongJi_DETAIL, rp, new MyRequestCallBack((BaseActivity) mActivity, MyConstans.FIRST));
	...
	private void initData(String pp_poorreason) {
		...
		MyApplication.getInstance().getHttpUtilsInstance().send(HttpMethod.POST, URLs.PinKunHuShuXingTongJi_DETAIL, rp
				, new MyRequestCallBack(mActivity, MyConstans.FIRST));
		System.out.println("请求困户属性统计详情的网址为：" + URLs.PinKunHuShuXingTongJi_DETAIL);
	}
	...
	public void onSuccessResult(String str, int flag) {
		...
		switch (flag){
			case MyConstans.FIRST:
				System.out.println("请求困户属性统计详情数据成功：==" + str);
...
//								expandTabView.setValue(mTextArray, mViewArray);
	...
	public void onFaileResult(String str, int flag) {
		...
		System.out.println("请求困户属性统计数据失败：==" + str);
	}
	...
	private void generateData(List<BasicPoorpeopleModel> lEntities) {
		String[] putong = {"行政区","贫困人口数","五保贫困户"};//五保贫困户
		String[] sangshi = {"行政区","贫困人口数","一般农户"};//一般农户
		String[] wu = {"行政区","贫困人口数","一般贫困户"};//一般贫困户
		String[] jineng = {"行政区","贫困人口数","低保贫困户"};//低保贫困户
