...
public class PinKunCunShuXingDetailActivity extends MainBaseActivity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		System.out.println("获取到点击饼状图区域的名字：==" + title);
		...
		if (title.contains("非贫困村")) {
			leixing = LX.fei;
			tv_title.setText("非贫困村");
			initData("非贫困村");
			case_detail="非贫困村";
		}else if (title.contains("三到村三到户项目村")) {
			leixing = LX.sandao;
			tv_title.setText("三到村三到户项目村");
			initData("三到村三到户项目村");
			case_detail="三到村三到户项目村";
		}else if (title.contains("经济薄弱村")) {
			leixing = LX.jingji;
			tv_title.setText("经济薄弱村");
			initData("经济薄弱村");
			case_detail="经济薄弱村";
		}else if (title.contains("十二五贫困村")) {
			leixing = LX.erwu;
			tv_title.setText("十二五贫困村");
			initData("十二五贫困村");
			case_detail="十二五贫困村";
		}else if (title.contains("十三五贫困村")) {
			leixing = LX.sanwu;
			tv_title.setText("十三五贫困村");
			initData("十三五贫困村");
			case_detail="十三五贫困村";
		}else if (title.contains("贫困村")) {
			leixing = LX.pinkun;
			tv_title.setText("贫困村");
			initData("贫困村");
			case_detail="贫困村";
		}
...
//		System.out.println("贫困人口统计页面一进来初始化时请求服务器中筛选条件的乡镇(flag=7)网址为：==" + URLs.TOWN + "?&ad_cd=" + ad_cd);
	...
	private void getData() {
		...
		if(flag_zhen_cun==1){
			if(StringUtil.isNotEmpty(adl_cd_zhen)){
				...
				MyApplication.getInstance().getHttpUtilsInstance().send(HttpMethod.POST,
						URLs.PinKunCunShuXingTongJi_DETAIL, rp, new MyRequestCallBack((BaseActivity) mActivity, MyConstans.FIRST));
				System.out.println("贫困人口统计请求服务器网址为：===" + URLs.PinKunCunShuXingTongJi_DETAIL);
			}
	...
	private void initData(String pp_poorreason) {
		...
		MyApplication.getInstance().getHttpUtilsInstance().send(HttpMethod.POST, URLs.PinKunCunShuXingTongJi_DETAIL, rp
				, new MyRequestCallBack(mActivity, MyConstans.FIRST));
		System.out.println("请求属性统计详情的网址为：" + URLs.PinKunCunShuXingTongJi_DETAIL);
	}
	...
	public void onSuccessResult(String str, int flag) {
		...
		switch (flag){
			case MyConstans.FIRST:
				System.out.println("请求属性统计详情数据成功：==" + str);
...
//								expandTabView.setValue(mTextArray, mViewArray);
	...
	public void onFaileResult(String str, int flag) {
		...
		System.out.println("请求属性统计数据失败：==" + str);
	}
	...
	private void generateData(List<BasicPoorpeopleModel> lEntities) {
		String[] fei = {"行政区","总贫困村数","非贫困村数"};//非贫困村
		String[] sandao = {"行政区","总贫困村数","三到村三到户项目村数"};//三到村三到户项目村
		String[] pinkun = {"行政区","总贫困村数","贫困村数"};//贫困村
		String[] jingji = {"行政区","总贫困村数","经济薄弱村数"};//经济薄弱村
		String[] erwu = {"行政区","总贫困村数","十二五贫困村数"};//十二五贫困村
		String[] sanwu = {"行政区","总贫困村数","十三五贫困村数"};//十三五贫困村

		switch (leixing) {
			case fei:  //非贫困村
			getBing(fei, lEntities);
				break;

			case sandao: //三到村三到户项目村
			getXue(sandao, lEntities);
				break;

			case pinkun: //贫困村
			getCan(pinkun, lEntities);
				break;

			case jingji: //经济薄弱村
			getNengli(jingji, lEntities);
				break;

			case erwu: //十二五贫困村
			getErWu(erwu, lEntities);
				break;

			case sanwu: //十三五贫困村
				getSanWu(sanwu, lEntities);
				break;
			...
			if (i == 0) {
				...
				for (int j = 0; j < case_bing_laodong_money_jishu.length; j++) {
					...
					case 2:
						row.setGravity(Gravity.CENTER);
						row.setLayoutParams(getLayoutParams(1,1,1,0));
						row.addView(getLinearLayout(getTextView(String.valueOf(lEntities.get(i - 1).getBrcnum()))));
						break;
	 ...
	 */
	@SuppressLint("InflateParams")
	private void getSanWu(String[] case_bing_laodong_money_jishu, List<BasicPoorpeopleModel> lEntities) {
		for (int i = 0; i < lEntities.size() + 1; i++) {
			row = new TableRow(mContext);
			if (i == 0) {
				for (int j = 0; j < case_bing_laodong_money_jishu.length; j++) {
				...
				for (int j = 0; j < case_bing_laodong_money_jishu.length; j++) {
					switch (j) {
						...
						case 2:
							row.setGravity(Gravity.CENTER);
							row.setLayoutParams(getLayoutParams(1,1,1,0));
							row.addView(getLinearLayout(getTextView(String.valueOf(lEntities.get(i - 1).getSswpkcnum()))));
							break;
	 ...
	 */
	@SuppressLint("InflateParams")
	private void getErWu(String[] case_bing_laodong_money_jishu, List<BasicPoorpeopleModel> lEntities) {
		for (int i = 0; i < lEntities.size() + 1; i++) {
			row = new TableRow(mContext);
			if (i == 0) {
				for (int j = 0; j < case_bing_laodong_money_jishu.length; j++) {
				...
				for (int j = 0; j < case_bing_laodong_money_jishu.length; j++) {
					switch (j) {
						...
						case 2:
							row.setGravity(Gravity.CENTER);
							row.setLayoutParams(getLayoutParams(1,1,1,0));
							row.addView(getLinearLayout(getTextView(String.valueOf(lEntities.get(i - 1).getSewpkcnum()))));
							break;
			...
			if (i == 0) {
				...
				for (int j = 0; j < case_xue.length; j++) {
					...
					case 2:
						row.setGravity(Gravity.CENTER);
						row.setLayoutParams(getLayoutParams(1,1,1,0));
						row.addView(getLinearLayout(getTextView(String.valueOf(lEntities.get(i - 1).getScswpkcnum()))));
						break;
	...
	private void getBing(String[] case_bing_laodong_money_jishu, List<BasicPoorpeopleModel> lEntities) {
		for (int i = 0; i < lEntities.size() + 1; i++) {
			row = new TableRow(mContext);
			if (i == 0) { //第一行----表头
				for (int j = 0; j < case_bing_laodong_money_jishu.length; j++) { //遍历列数并赋值
					row.setGravity(Gravity.CENTER);
					row.setLayoutParams(getLayoutParams(1,1,1,0));
					row.addView(getLinearLayout(getTextView(case_bing_laodong_money_jishu[j])));
				}
			}else if (i > 0 && i!=lEntities.size()) { //第二行至倒数第二行-----数据内容
				for (int j = 0; j < case_bing_laodong_money_jishu.length; j++) { //遍历列数并赋值
					switch (j) { //赋值时需要减1，因为有了第一行，减1是为了从下表为0开始取值
					case 0: //第一列
						row.setGravity(Gravity.CENTER);
						row.setLayoutParams(getLayoutParams(1,1,1,0));
						row.addView(getLinearLayout(getTextView(lEntities.get(i - 1).getAdl_nm())));
					...
					case 2: //第三列
						row.addView(getLinearLayout(getTextView(String.valueOf(lEntities.get(i - 1).getNopkcnum()))));
						row.setLayoutParams(getLayoutParams(1,1,1,0));
						row.setGravity(Gravity.CENTER);
						break;
					default:
			...
			if (i == 0) {
				...
				for (int j = 0; j < case_can.length; j++) {
					...
					case 2:
						row.setGravity(Gravity.CENTER);
						row.setLayoutParams(getLayoutParams(1,1,1,0));
						row.addView(getLinearLayout(getTextView(String.valueOf(lEntities.get(i - 1).getPkcnum()))));
						break;
